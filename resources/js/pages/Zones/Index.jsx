...
export default function Index({ zones }) {
            ...
            col.display({
                ...
                cell: ({ row }) => (
                    <div className="flex items-center gap-2">
                        <Link
                            href={route('zones.show', row.original.id)}
                        ...
                        <Link
                            href={route('zones.edit', row.original.id)}
    ...
    function handleDelete() {
        router.delete(route('zones.destroy', deleting.id), {
            onFinish: () => setDeleting(null),
        });
    }

    return (
        <AuthenticatedLayout header="Zonas">
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-xl font-semibold text-gray-800">Zonas</h1>
                    <div className="flex gap-2">
                        <Link
                            href={route('zones.trashed')}
            ...
            <ConfirmDeleteModal
                ...
                title="Eliminar zona"
                message={`¿Eliminar la zona "${deleting?.name}"?`}
            />
        </AuthenticatedLayout>
    );
}
